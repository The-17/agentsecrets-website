...
**How it works:**
- `secrets set` → writes directly to OS keychain
- `secrets push` → reads from `.env.{environment}` (falls back to `.env`), encrypts, uploads to cloud
...
Mode 2 creates `.env.{environment}` files on disk after each pull. These files contain plaintext values. Add `.env.*` to your `.gitignore`, treat these files with the same care as any file containing credential values, do not use mode 2 in projects where an AI agent has filesystem access to the project directory, and delete the files when they are no longer needed rather than leaving them on disk indefinitely.
