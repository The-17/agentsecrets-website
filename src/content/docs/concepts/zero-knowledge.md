...
The first: the agent never sees the credential value. The AI agent, the calling code, and the calling process receive only the API response. The credential value is resolved by the proxy in its own process and injected at the transport layer. It does not cross into the agent's context at any point.
